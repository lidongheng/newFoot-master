...
- 支持让球、大小球、独赢三种玩法。
- 用户端每 5 秒刷新滚球列表，页面隐藏或离开后停止轮询。
- 后台通过比赛列表的“滚球”操作统一更新比分、分钟、阶段、赔率和开盘状态。
- 滚球下单由服务端校验比赛状态、开盘状态和盘口版本，订单保存服务端成交快照。
- 发生变盘时返回最新盘口，用户接受后必须再次确认下注。
- 状态切换为进行中时保持封盘，必须由管理员显式开盘。
