...
def get_stock_news(
    ...
    Returns:
        List[Dict]: 新闻列表
    """
    cache_key = f"news_{stock_code}_{days}_{max_news}"
    ...
    # ===== 直接调用东方财富搜索API（不依赖akshare的内部pandas） =====
    try:
        import requests
        import json
        ...
        if resp.status_code == 200:
            ...
            if isinstance(data, dict):
                # 东方财富API返回的嵌套结构: result.cmsArticleWebOld
                result_data = data.get("result", {})
                if isinstance(result_data, dict):
                    for art_key in result_data:
                        arts = result_data[art_key]
            ...
            for art in articles:
                ...
                content = str(content) if content else title
                ...
                for fmt in ("%Y-%m-%d %H:%M:%S", "%Y-%m-%d", "%Y/%m/%d %H:%M:%S",
                            "%Y-%m-%dT%H:%M:%S", "%Y-%m-%d %H:%M", "%Y年%m月%d日 %H:%M"):
                    try:
                        pub_time = datetime.strptime(str(pub_time_str)[:19], fmt)
                        break
                ...
                news_list.append({
                    ...
                    "source": str(source).strip() if source else "东方财富",
                    "url": str(art_url).strip() if art_url else "",
                })
    except Exception as e:
        print(f"  首次新闻API获取失败: {e}")

    # ===== 如果没获取到，尝试AKShare =====
    if not news_list:
        try:
            import akshare as ak
            # 使用低层HTTP调用，绕过pandas解析
            news_df = ak.stock_news_em(symbol=stock_code)
            if news_df is not None and len(news_df) > 0:
                news_list = _parse_akshare_news(news_df, days)
        except Exception:
            pass
